...
extern uint8_t volatile g_u8EPBReady;
uint8_t g_u8OutBuff[64];
uint8_t g_u8InBuff[64];
extern void ONLINE_COMMAND(void);
void USB_IN_DATA(void)
{
	int i;
for (i=0; i<64; i++)
        HSUSBD->EP[EPA].EPDAT_BYTE = g_u8InBuff[i];
        HSUSBD->EP[EPA].EPTXCNT = 64;
