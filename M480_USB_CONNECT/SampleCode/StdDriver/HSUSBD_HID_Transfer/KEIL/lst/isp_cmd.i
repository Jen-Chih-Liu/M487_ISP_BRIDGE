...
__align(4) uint8_t rcvbuf[64];
__align(4) uint8_t sendbuf[64];
...
ErrNo RcvData(void)
{
  ...
  Result=io_read(DEV_handle, rcvbuf, &len);
	...
	if ((*((volatile unsigned int *)(pBuf))) != g_packno)
	{
		printf("g_packno=%d rcv %d\n", g_packno, (*((volatile unsigned int *)(pBuf))));
		Result = 3;
	}
	else
	{
